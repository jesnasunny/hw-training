...
class IntersparScraper:
    def __init__(self, base_urls_with_paths, max_pages=50, mongo_uri='mongodb://localhost:27017/', db_name='interspar_database', collection_name='product_urls'):
    ...
    def extract_urls(self, data, base_path):
        ...
        urls = []
        for item in items:
            product = hit.get('product_data', {})
            relative_url = product.get('url', '')
            if relative_url:
                full_url = base_path + relative_url
