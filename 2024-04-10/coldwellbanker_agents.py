...
def fetch_datas(base_url, last_url):
    ...
    try:
        ...
        if agent_details:
            ...
            languages = agent_details.get('languages', [{'longDescription': 'Not specified'}])
            if languages:
                agent_data['languages'] = languages[0].get('longDescription')
            else:
                agent_data['languages'] = 'Not specified'
